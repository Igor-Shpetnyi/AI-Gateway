...
src/lib/
  ...
  router.ts       provider selection loop, per-provider key rotation + request logging (exports logRequest)
  providers/
...
src/server/
  ...
  routers/providerKeys.ts add/remove/toggle API keys per provider (masked in responses, never returns plaintext)
